...
async function creatComment(commentType: CommentType = {}): Promise<IComment| null> {
 
  const result = await Comment.create(commentType)
  return result;
  
}
async function getCommnent(id: string, page: number, size: number): Promise<IComment| null> {
...
export { getCompaniesPaginated,getDetailCompany,creatComment, getCommnent, creatLike,getTotalLike };
